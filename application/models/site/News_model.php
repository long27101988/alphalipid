...
class News_model extends CI_Model{
	...
	public function get_news_by_cat_id_limit($id, $offset, $size){
		$this->db->where('cat_id', $id);
	...
	public function search_news_limit($id_cate, $txtSearch, $offset, $size){
		$arrCondition = explode(' ', $txtSearch);
		$arr_result = array();
		$arr_result = $this->get_data_by_search_limit((int)$id_cate, strtolower($txtSearch), $offset, $size);
		if(count($arr_result) <= 0){
			foreach ($arrCondition as $key => $valSearch) {
				$array_temp = $this->get_data_by_search_limit((int)$id_cate, strtolower($valSearch), $offset, $size);
				$arr_result = array_merge_recursive($arr_result, $array_temp);
			}	
		}
		return $arr_result;
	...
	public function get_data_by_search_limit($id_cate, $textSearch, $offset, $size){
		$this->db->where('cat_id', $id_cate);
		$this->db->like('title', $textSearch);
		$this->db->or_like('`desc`', $textSearch);
		$this->db->or_like('content', $textSearch);
